...
class AbstractNode extends EventEmitter {
  ...
  static get DEFAULT_SEND_OPTIONS() {
    return {
      timeout: constants.T_RESPONSETIMEOUT
    };
  }
   ...
   * @param {object} [options]
   * @param {number} [options.timeout=T_RESPONSETIMEOUT] - Override timeout
   * @param {AbstractNode~sendCallback} [callback]
   */
  send(method, params, target, options, handler = () => null) {
    ...
    if (typeof options === 'function') {
      handler = options;
      options = AbstractNode.DEFAULT_SEND_OPTIONS;
    } else {
      options = merge(AbstractNode.DEFAULT_SEND_OPTIONS, options);
      assert(options.timeout >= constants.T_RESPONSETIMEOUT,
             'Timeout value supplied is less than T_RESPONSETIMEOUT');
    }
